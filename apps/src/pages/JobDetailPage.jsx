...
const JobDetailPage = () => {
  ...
  const { id } = useParams(); // Lấy số 1, 2 hoặc 3 từ thanh địa chỉ
  const [job, setJob] = useState(null); // State lưu data từ Neon
  const [loading, setLoading] = useState(true);

  // Hàm lấy chi tiết Job từ Backend
  const fetchJobDetail = async () => {
    if (!id) { setLoading(false); return; } // Nếu không có ID thì hiện data mẫu Google
    try {
      const res = await fetch(`http://localhost:5000/api/job-detail/${id}`);
      const result = await res.json();
      if (result.success) setJob(result.data);
    } catch (err) { console.log("Lỗi load detail:", err); }
  ...
  const [applyForm, setApplyForm] = useState({
    ...
    email: 'kimngannguyenanh@gmail.com',
    phone: '',
    coverLetter: '',
    cvFile: null
  });
  ...
  useEffect(() => {
    ...
    if (savedData) {
        ...
        // --- GỌI API LẤY THÔNG TIN CÁ NHÂN THẬT ---
        const fetchUserData = async () => {
            try {
                ...
                if (result.success) {
                    setApplyForm(prev => ({
                        ...prev,
                        fullName: result.data.full_name, // Lấy từ DB
                        email: result.data.email       // Lấy từ DB
                    }));
                }
            } catch (err) {
  ...
  const handleSubmitApplication = async (e) => {
    ...
    formData.append('candidateId', savedData.id);
    ...
    try {
        const res = await fetch('http://localhost:5000/api/applications', {
            method: 'POST',
            body: formData, // Không để Header Content-Type vì FormData tự lo
        });
        const result = await res.json();
        if (result.success) {
            setShowApplyModal(false);
            alert("🎉 Nộp hồ sơ thành công! Nhà tuyển dụng sẽ sớm liên hệ qua SĐT: " + applyForm.phone);
        }
    } catch (err) {
        console.error("Lỗi nộp hồ sơ:", err);
        alert("Có lỗi xảy ra, ck kiểm tra lại server nhé!");
    }
};
          ...
          <div className="left-column space-y-6">
            ...
            <div className="card job-header-card">
              ...
              <div className="header-content-relative">
                <div className="header-top-row">
                  <div style={{ flex: 1 }}>
                    <h1 className="job-title">{job ? job.title : 'Senior UI/UX Designer'}</h1>
                    <div className="job-meta-row">
                      <div className="meta-item">
                        <Building2 size={18} className="text-blue-600" />
                        <span className="company-name">{job ? job.company_name : 'Google Inc.'}</span>
                      </div>
                      <div className="meta-item">
                ...
                <div className="stats-grid">
                  <div className="stat-card stat-blue">
                    <div className="stat-content">
                      <div className="stat-icon-wrapper bg-blue-600"><DollarSign size={24} className="text-white" /></div>
                      <div>
                        <p className="stat-label text-blue-700">Mức lương</p>
                        <p className="stat-value text-blue-900">{job ? job.salary : '$2,000 - $3,500'}</p>
                      </div>
                    </div>
                  </div>
                  <div className="stat-card stat-purple">
                    <div className="stat-content">
                      <div className="stat-icon-wrapper bg-purple-600"><MapPin size={24} className="text-white" /></div>
                      <div>
                        <p className="stat-label text-purple-700">Địa điểm</p>
                        <p className="stat-value text-purple-900">{job ? job.location : 'Hồ Chí Minh'}</p>
                      </div>
                    </div>
                  </div>
                  <div className="stat-card stat-pink">
                    <div className="stat-content">
                      <div className="stat-icon-wrapper bg-pink-600"><Briefcase size={24} className="text-white" /></div>
                      <div>
                        <p className="stat-label text-pink-700">Kinh nghiệm</p>
                        <p className="stat-value text-pink-900">{job ? job.experience : '3 - 5 Năm'}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="card">
              ...
              <div className="section-text">
                {/* SỬA Ở ĐÂY: Hiện job.description từ database */}
                <p style={{ whiteSpace: 'pre-line' }}>
                  {job ? job.description : '- Chịu trách nhiệm thiết kế giao diện (UI)...'}
                </p>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="section-text">
                <p style={{ whiteSpace: 'pre-line' }}>
                  {job ? job.requirements : '- Có ít nhất 3 năm kinh nghiệm ở vị trí tương đương...'}
                </p>
              </div>
            </div>
